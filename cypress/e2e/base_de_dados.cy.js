...
describe('Workflow da base de dados', () => {
  ...
  it('should navigate through each Demanda tab and display data', () => {
    const demandas = ['Psicologia', 'Jurídico', 'Assistência social', 'Abrigamentos'];

    demandas.forEach(demanda => {
      cy.contains(demanda).click();
      cy.url().should('include', demanda.toLowerCase().replace(' ', '-'));
      cy.get('th:contains("Status - ' + demanda + '")').should('be.visible');
